...
- `OperativeLifeContracts` — shared stable operative life IDs and server-authoritative health, transition, revive, solo-recovery, and squad-viability data shapes; declarations only in LK-0301
...
The declaration layer and pure LK-0302 through LK-0304 resolvers have no runtime owner and are not imported by either bootstrap. LK-0302 copies a caller-owned processed-damage-event set and requires atomic returned-snapshot commit, but defines no cleanup or persistence. LK-0303 synchronously evaluates stored bleed-out and solo-recovery deadlines and accepted finishing damage. LK-0304 accepts server-owned distance, line-of-sight, connection/participation, movement, and accepted-damage continuity facts; it derives continuous four-second revive progress, resets interruption to a copied cancelled session, and restores the configured `30` health on completion. Its focused caller-owned combat companion reuses canonical P2 weapon, selection, and ShotId types to preserve copied ammunition/ShotIds while clearing reload and selected target. These modules add no scheduler, clock or session owner, P2 orchestration, remotes, UI, or character mutation. They do not reinterpret the P2 test-hostile `TargetHealthState`, change `DamageResolver.becameDead`, or own processed ShotIds.

## Change policy

Architecture exists to support the next playable milestone. A new abstraction must solve a present, demonstrated need or remove meaningful duplication. P1 is complete and remains unchanged. LK-0201 defines shared automatic-combat contracts and prototype firearm configuration without runtime behavior. LK-0202 adds only deterministic server target-candidate validation without runtime integration. LK-0203 adds only pure deterministic target selection without runtime integration. LK-0204 adds only a pure server-owned automatic-fire decision and weapon-state transition without runtime integration. LK-0205 adds only pure server-owned hit revalidation and health-state transitions without runtime integration. LK-0206 adds a focused local `WeaponController`, pure reload transitions, and explicit reload/presentation remotes. LK-0207 completes P2 with focused two-client security, integration, presentation, and feel validation through a Studio-only stationary-fixture harness. The harness is not a production combat owner and cannot run outside Studio. LK-0301 adds shared P3 declarations and centralized configuration only. LK-0302 adds only pure operative damage and `Alive`-to-`Incapacitated` transitions without runtime integration. LK-0303 adds only pure incapacitated bleed-out, finishing-death, and solo-recovery transitions without runtime integration. LK-0304 adds only pure revive eligibility, progress, interruption reset, and completion without runtime integration. LK-0305 is next and remains unstarted.
