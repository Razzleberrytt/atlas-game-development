...
The declaration layer and pure LK-0302 through LK-0304 resolvers have no runtime owner and are not imported by either bootstrap. LK-0302 copies a caller-owned processed-damage-event set and requires atomic returned-snapshot commit, but defines no cleanup or persistence. LK-0303 synchronously evaluates stored bleed-out and solo-recovery deadlines and accepted finishing damage. LK-0304 accepts server-owned distance and line-of-sight facts, derives continuous four-second revive progress, resets interruption to a copied cancelled session, and restores the configured `30` health on completion. These modules add no scheduler, clock or session owner, remotes, UI, or character mutation. They do not reinterpret the P2 test-hostile `TargetHealthState`, change `DamageResolver.becameDead`, or own processed ShotIds.
