...
## P5-0101 world-foundation validation

A Windows 11 Roblox Studio two-client Server & Clients session synchronized through Rojo 7.7.0 at `localhost:34872`. The server generated one `LivingKingdomsWorld`, printed `[Living Kingdoms] World foundation ready: P5-0101-v1`, and then printed the normal server bootstrap. Both clients printed the existing camera and client bootstrap confirmations. No new Living Kingdoms warning or error appeared.

The first visual run exposed that characters still used the legacy place spawn, leaving the tactical camera at the old combat-harness origin. `WorldFoundationService` now assigns the generated ranger-station `SquadInsertion` to every existing and joining player's `RespawnLocation` before `OperativeLifeService` loads characters, and removes only its own assignment on stop. The repeated run framed the ranger station, nearby canopy, ground, and red emergency light from the authored southwest insertion. Forest and ridge silhouettes remained readable without the flashlight, while local light created a stronger navigation cue. The ambient floor was raised modestly after the initial darkness check; the final scene retained a dark blue-gray night identity.

The server viewport showed the generated mixed forest, road/terrain bands, elevation silhouettes, and bounded local-light contrast. Runtime source generation and client ambience started cleanly in a two-player session. The automated pass did not drive a complete on-foot circuit through all eight landmarks, assess final art quality, profile GPU/render cost, or test mobile/console presentation. The pre-existing LK-0207 hostile development fixtures remained visible around the old origin; P5-0101 introduced no enemy type or enemy behavior.

## P5-0102 environment-readability validation

- Environment: Windows 11, Roblox Studio two-client local Server & Clients session, repository-pinned Rojo 7.7.0 build.
- World version: `P5-0102-v1`.
- Automated validation: all 29 existing Lune fixtures passed; scoped StyLua and Selene passed for every modified Luau file; Rojo sourcemap and build completed successfully.
- Runtime footprint: `973` generated descendants, including `931` base parts and `11` lights, with `0` particle emitters and `0` scripts inside `LivingKingdomsWorld`.

The clean server generated exactly one `LivingKingdomsWorld` and printed `[Living Kingdoms] World foundation ready: P5-0102-v1` before the normal bootstrap. Two clients registered Alive at `100/100` and observed the same ranger-station insertion composition. The warm generator pool, dark headquarters roof, radio mast, truck silhouette, foreground trail surface, and surrounding canopy remained separable from the gameplay camera. The second client remained isolated and stable while server-side review positions were exercised on the first character.

The runtime audit confirmed eight attributed landmark models, route-aware vegetation clearance in source, no environment-owned gameplay script, no particle cost, and a deliberately small local-light set. The environment still uses the existing 10 Hz client ambience loop; P5-0102 added one aviation-blink tag but no additional scheduler or connection. The deterministic vegetation budget is lower than P5-0101 while new landmark/story props bring the generated part count to 931. No profiler capture or lower-end hardware test was performed, so this count is an implementation observation rather than a frame-rate guarantee.

Studio command-bar `Humanoid:MoveTo` and direct velocity probes were rejected or neutralized by the existing custom client movement controller and server movement validation, as expected for non-gameplay input. Repeated real keyboard taps did move the character through the production input path, but the available automation interface could not hold directional keys long enough to complete a continuous human-paced circuit. Server-authoritative inspection positions successfully placed a review character at landmark coordinates, but the automated client-camera capture did not reliably retarget after those live-only anchor moves. Therefore the two-client startup, insertion camera, lighting/atmosphere, generated footprint, and production-input response are verified; a complete human-held-key on-foot circuit and landmark-by-landmark capture remain required before calling final art validation complete.

The clients emitted Roblox-owned `ChatScript` `SetCoreGuiChatConnections has not been registered by the CoreScripts` messages during local client startup. No new warning or error originated from Living Kingdoms source. The pre-existing Studio-only LK-0207 hostile fixtures remained visible in the server viewport and were not modified by P5-0102.

## P5 enemy-pressure and combat-runtime validation

- Environment: Linux repository validation container; repository-pinned StyLua 2.5.2, Selene 0.31.0, Rojo 7.7.0, and Lune 0.10.5.
- Enemy configuration version: `P5-0104-v1`.
- Automated validation: all 36 Lune fixtures passed, including the five new P5 fixtures (`EnemyContracts`, `EnemyBehaviorResolver`, `EnemyDirectorService`, `OperativeCombatRuntimeService`, `P5IntegrationValidation`); StyLua and Selene passed for `games/living-kingdoms/src` with zero errors and zero warnings; `rojo sourcemap` and `rojo build` completed successfully.
- Measured bounds (fixture-verified): population caps of 6/12/24 concurrent enemies for 1/2/4 registered operatives; exactly one enemy-director heartbeat and one combat-runtime heartbeat; zero per-enemy connections, timers, raycasts, or remotes in the director; at most three line-of-sight raycasts per operative per combat evaluation; a 16-entry bound on deferred fair spawns.

The P5 runtime was validated through repository fixtures only. The end-to-end fixture drives the real `MissionDirectorService` against the real `EnemyDirectorService`: infiltration begins level-0 roaming pressure, all three authored waves spawn production walkers at their authored beats, the retuned holdout wave spawns immediately from outside the fair-spawn floor while a survivor holds the extraction center, success and squad-wipe resolutions both stand every enemy down, and no client path can spawn, steer, damage, or reveal an enemy.

A live multiplayer Roblox Studio playthrough remains required and has not been performed for P5-0104 through P5-0107. The outstanding manual script: start a two-client session, confirm walkers roam near the squad on the infiltration route and pursue when noticed, verify automatic fire acquires and kills a pursuing walker (three basic-firearm hits) with the target highlight and corpse cleanup, take a melee hit and confirm authoritative health loss plus revive flow after incapacitation, complete the relay objective and observe wave 1 on the descent, hear "pressure rising" with wave 2 at the creek, hold the clearing while the wave-3 walkers converge from outside the darkness radius, and confirm every walker stands down on the success card. Pacing feel, movement quality through dense graybox forest, and horde frame cost on representative hardware are explicitly unmeasured until that session.

## 2026-07-21 P6 Studio startup, scaling, and presentation probes

- Environment: Windows 11, Roblox Studio local Server & Clients, Rojo 7.7.0 at `localhost:34872`.
- Configuration: `P6-0101-v1` scarcity and `HROI-0120-v1-server-owned-weapon-roster` firearms.
- A reproducible Studio-only startup race was repaired. The server previously spent the eight-second insertion window before its first slow local client connected, froze an empty roster, and immediately resolved `Squad Consumed`. Insertion now arms on the first connected player; duplicate mission/class bootstrap calls were also removed. A regression fixture proves empty-server deferral, first-player arming, and exactly one start call per owner.
- Fresh 1-, 2-, and 4-client sessions admitted exactly 1/2/4 Alive operatives and advanced to the active relay objective. This removes the workflow blocker that prevented meaningful multiplayer evidence collection.
- Read-only ammunition probes observed 3/6/12 independent cache opportunities and the correct aggregate roster sizes. The clients remained stationary, did not collect caches, and did not issue reload intent, so their late ammunition snapshots are explicitly invalid for balance comparison. The calculated demand/supply projection and exact snapshots are recorded in `docs/specifications/ammunition-scarcity-and-supply.md`.
...
## P7-0104 Medic validation checklist

P7-0104 is fixture-validated in source; a live Studio session has not been performed for this slice. In a two-client Server & Clients session, select Medic before insertion, damage the teammate without incapacitating them, and press the class-action input near the teammate. Confirm the three-second channel visibly targets that teammate, moving more than 1.5 studs, taking damage, losing range/line of sight, or the target becoming invalid interrupts it, and a successful uninterrupted channel restores at most 25 health while reducing the Medic's visible charge count from three to two. Repeat until charges reach zero and confirm further treatment fails without changing health.

Then incapacitate the non-Medic teammate and hold the ordinary revive input as the Medic. Confirm the existing P3 revive disclosure identifies the session as medic-assisted, completes in approximately three seconds rather than four, restores exactly 40 health, preserves ammunition/cadence history, and grants no invulnerability. Repeat with a non-Medic reviver and confirm the original four-second/30-health path remains intact. Attempt self-treatment, treatment of a healthy/incapacitated/dead teammate, malformed action payloads, and release/cancel races; no case may fabricate health, spend negative charges, revive Dead operatives, or produce a Living Kingdoms-originated warning/error.

## P7-0105 Engineer validation checklist

P7-0105 is fixture-validated in source; a live Studio session has not been performed for this slice. In a two-client Server & Clients session, select Engineer before insertion, fire enough rounds on the nearby teammate to create reserve capacity, and press the class-action input while within eight studs. Confirm the 2.5-second channel visibly targets the teammate, moving more than 1.5 studs, taking damage, losing range/line of sight, or invalidating either operative interrupts it, and a successful channel grants at most 12 reserve rounds while reducing the Engineer's visible charge count from two to one. Repeat for the second charge, confirm a third attempt cannot create ammunition, and verify partial capacity grants only the missing rounds rather than exceeding the weapon's reserve cap.

Repeat with each selectable weapon and confirm the equipped server-owned weapon ID determines compatibility/capacity while the client never supplies weapon, reserve, grant, or timestamp values. In solo Studio, confirm the same action falls back to the Engineer as its target. Attempt to resupply a full-reserve, incapacitated, dead, disconnected, out-of-range, or obstructed operative and replay accepted request IDs; no attempt may spend a charge without a positive commit, reuse an authored cache, duplicate a supply, or alter objective state. Capture one scarcity telemetry sample and confirm Engineer resupply count/rounds are reported separately from authored-cache grants. No Living Kingdoms-originated warning/error should appear.

## P7-0106 cross-class presentation validation checklist

P7-0106 is fixture-validated in source; a live Studio session has not been performed for this slice. In a three-client Server & Clients session, select one of each starting class and allow insertion to lock. Confirm every client sees teammate labels with a written shape token (`[SHIELD]`, `[CROSS]`, or `[TOOL]`), the full class name, and a role summary; identity must remain understandable in grayscale and with audio muted. The owner panel must show the same class identity, current/maximum Medic or Engineer charges (or the Combat Specialist's cooldown limitation), action input, numeric active time, numeric cooldown time, and concise human-readable rejection copy rather than internal reason IDs.

Activate Brace, Field Treatment, and Field Resupply in turn. Confirm other clients see the acting teammate's label change to the named action and gain a thicker outline without revealing unseen enemies, objective state, supply capacity, target health, or private cooldown details. Exercise movement, damage, reload, range, line-of-sight, full-health, full-reserve, incompatible-weapon, depleted-resource, and rapid-input failures and confirm the owner receives the matching concise reason. Replace a character, disconnect a teammate, resolve the operation, and restart; no stale billboard, highlight, countdown, input binding, or duplicate label may remain. Record this composed presentation pass with the wider P7-0107 solo/duplicate/balanced matrix rather than claiming it from automated fixtures.

## 2026-07-21 P7-0107 validation evidence

- Automated validation passed all 143 Lune fixtures. The P7 security/scaling audit covers sender-derived identity, forged facts, locked swaps, replay/stale requests, cooldown and resource conservation, duplicate roles, absence-of-role viability, disconnect cleanup, bounded records, and the absence of any client mutation or role gate.
- `ClassValidationTelemetryService` records authoritative availability, starts, completions, interruptions, cooldown/active time, finite resources, healing, and ammunition grants in an eight-operative bounded ledger. `ClassValidationProbeService` exposes immutable snapshots only through Studio-only `ServerStorage` bindables; production clients receive no probe remote or mutation surface.
- A Windows 11 Roblox Studio local Server & Clients run launched one server and four isolated clients from the generated P7 build. The server admitted all four Alive operatives, advanced Operation Blackwater Relay, and printed the world foundation, mission director, and server bootstrap confirmations without a Living Kingdoms-originated warning or error. All four clients reached the live objective and displayed the four-member squad state.
- Studio emitted four editor-owned `StyleRule` conversion messages for its `RoundedCorner8` rule; the message identifies Studio styling rather than Living Kingdoms source and did not interrupt either bootstrap.
- No balance values or human feel conclusions are inferred from the stationary automation pass. The bounded telemetry surface is retained for later tuning sessions; P12 still owns repeated measured balance validation.
