...
P7 is **complete** (`P7-PLAN-001` through `P7-0107`). P8 is implemented through `P8-0107` (see [`../specifications/authored-objective-chain.md`](../specifications/authored-objective-chain.md)); `P8-0108` automated coverage is complete and only its live Studio playtest remains. `P9` is **complete** (`P9-PLAN-001` through `P9-0106`; see [`../specifications/special-enemy-and-boss-encounter.md`](../specifications/special-enemy-and-boss-encounter.md)). `P10-PLAN-001` through `P10-0106` are **complete** and `P10-0107`'s automated half has landed (see [`../specifications/match-completion-and-result.md`](../specifications/match-completion-and-result.md); the `MatchResultContracts`/`MatchResultConfig` vocabulary and the pure `TerminalResultResolver` are implemented); the only implementable P10 work left is the `P10-0107` Studio matrix, which is a manual gate; `RPG-0112` is now unblocked by the P10 result owner.
...
- [x] **P10-PLAN-001 — Specify terminal operation flow and result semantics.**
  ...
  - **Complete** — [`../specifications/match-completion-and-result.md`](../specifications/match-completion-and-result.md) fixes the terminal causes (`Success`/`Extracted` on boss defeat + extraction, `Failure`/`SquadWipe`, `Failure`/`Abandoned`) all converging on one first-commit-wins resolver with a wipe-outranks-extraction precedence; the boss-defeat → extraction-arrival-window sequence (wiring the P9-0104 deferral); the result and per-operative contribution fact vocabulary (recorded server-side for P11, no XP in P10); the documented cleanup stop order with a fresh per-run `operationId` replay reset; the session-retention/rejoin/abandonment policy keyed by operative entity ID; and the safe result snapshot disclosure. Every decision is mapped to `P10-0101`–`P10-0107`.
- [x] **P10-0101 — Define match-result and extraction contracts/configuration.**
  ...
  - **Complete** — `src/shared/Mission/MatchResultContracts.luau` fixes the `MissionResultCauseId` (`Extracted`/`SquadWipe`/`Abandoned`) vocabulary and its fixed cause→outcome pairing, the `ExtractionReadinessStateId` (`Locked`/`Inbound`/`Arrived`), `OperationReplayStateId`, and `CleanupOwnerId` vocabularies, and the safe result and per-operative contribution snapshot shapes (kills, damage, revives, objectives, class actions, boss exposure hits, survival state) with validators — reusing the existing `MissionContracts` outcome/phase vocabulary and the P3 `OperativeLifeContracts` life state, and representing no XP/reward. `src/shared/Config/MatchResultConfig.luau` holds the extraction arrival window and the documented cleanup stop order (enemies first, mission director last; every owner exactly once) with invariants. Pure declarations and fixtures only (`tests/MatchResultContracts.test.luau`); no runtime.
- [x] **P10-0102 — Implement one authoritative terminal-result resolver.**
  ...
  - **Complete** — `src/server/Systems/TerminalResultResolver.luau` is the one pure, deterministic decision: given the server-owned facts (`resolvedCauseId`, `squadWiped`, `isAbandoned`, `extractionArrived`, timestamp) it returns the single terminal result to commit, or a reason the operation is not terminal. Fixed precedence (`SquadWipe` > `Abandoned` > `Extracted` — a dead squad cannot extract) resolves simultaneous conditions to exactly one result, and a non-nil `resolvedCauseId` yields `AlreadyResolved`, so re-evaluation after a commit never produces a second result. Objectives stay non-terminal and there is no timeout cause (per the plan). `MatchResultContracts` gained the `TerminalDecisionRejectionReasonId`/`TerminalDecision` vocabulary. Fixtures: `tests/TerminalResultResolver.test.luau` (validation, precedence, first-commit-wins idempotence, not-terminal, determinism). No runtime wiring yet (`P10-0103`).
- [x] **P10-0103 — Integrate the final extraction or holdout sequence.**
  ...
  - **Complete** — the holdout begins with extraction `Locked` and no countdown, so a living Progenitor cannot be waited out. Its authoritative defeat (`readBossState().isDefeated`) opens the `MatchResultConfig.ExtractionArrivalWindowSeconds` window with the new `ExtractionInbound` radio line; one admitted, Alive operative inside the clearing at the deadline moves readiness to `Arrived`, the only fact feeding the resolver's `extractionArrived`. Squad failure and abandonment became facts rather than direct outcomes, so a wipe resolves `SquadWipe` and an all-disconnect `Abandoned` through the same single `TerminalResultResolver` decision, with the committed cause fed back so re-evaluation is inert. Late entry, early departure, incapacitation, death, and disconnect all resolve through the one presence rule, and a missed extraction is deliberately not a failure — the operation stays unresolved until someone is present. Fixtures: `ExtractionSequenceIntegration.test.luau` plus extended mission and P5 integration scenarios.
- [x] **P10-0104 — Add result presentation.**
  ...
  - **Part 1 complete** — `MatchResultService` is the bounded, inert per-operative contribution ledger (no remote, connection, timer, or gameplay authority). The owners that already observe the committed events record into it: combat (damage, kills, boss-exposure hits via the new `EnemyDirectorService.isBossEntity`), revive, class, and mission objectives. At the single terminal boundary the mission director hands over its squad facts — including the phase actually reached — and the ledger freezes one validated `SafeMatchResultSnapshot` with the P3 survival state per operative. First commit wins, an invalid snapshot is refused, the population and counters are bounded above four operatives, and the snapshot carries exactly the P10-0101 fields so no XP/rank/unlock can ride along. Fixture: `MatchResultContribution.test.luau`. **Part 2 complete** — the existing `MissionNetwork` gained one server→client `Result` channel and no request surface (the mission owner still accepts nothing). Resolution discloses the frozen snapshot once, and a client arriving afterwards is pushed the same snapshot on join. `MatchResultController` renders only a validator-accepted payload, exactly once: plain-language outcome and cause per authored cause, the key events, and every operative's contribution row with the local operative marked and survival state in words. No XP/rank/unlock and no replay control — replay lands in `P10-0105`, so the screen states plainly that nothing has been awarded rather than offering a dead button. Fixture: `MatchResultPresentation.test.luau`.
- [x] **P10-0105 — Implement deterministic match cleanup and replay.**
  ...
  - **Complete** — `OperationLifecycleService` is the one owner of the terminal stop/start sequence and holds no gameplay state: it executes `MatchResultConfig.CleanupStopOrder` and the new `ReplayStartOrder` against each system's existing lifecycle, both asserted to cover every declared cleanup owner exactly once. The start order is deliberately not the reverse of the stop order (life and roster owners run before the mission restarts), and the objective runtime releases at its documented place via `MissionDirectorService.resetObjectiveRuntime()` while the director tears the rest down last. Replay is server-driven: the owner subscribes to the single terminal commit, holds `Resolved` for `MatchResultConfig.ReplayDebriefSeconds`, then restarts — one timer, one subscription, both released on teardown, and no remote, so a client can neither request nor delay it. The mission director now mints a per-run `operationId` (`…:run-N`) on every start, published and recorded, so a replay is a distinct result rather than a resume. Fixture: `OperationLifecycleReplay.test.luau`.
  - **2026-08-08 acceptance repair** — a recorded-artifact playthrough exposed that the original replay stopped the application-scoped life owner, erased long-lived consumers, let client revision filters reject run 2, and left the prior debrief visible. `OperativeLifeService.resetForReplay()` now resets operation state without disconnecting application subscribers; mission HUD/world-object consumers compare revisions within `operationId`; and the debrief hides/re-arms on the next `Insertion`. Pinned CI artifact `9028866465` proved run 2 at 100 health, WalkSpeed 16, debrief hidden, and the opening objective advancing again. Evidence: [`../production/evidence/2026-08-08-r1-playable-replay-loop.md`](../production/evidence/2026-08-08-r1-playable-replay-loop.md).
- [x] **P10-0106 — Complete leave, disconnect, rejoin, and shutdown handling.**
  ...
  - **Complete** — retention lives in the owner that holds each fact, keyed by the server-owned operative entity ID, so there is no session hub and no `PlayerRemoving` ordering hazard. An audit found three real gaps, now closed: the life owner discarded its entry (reconnecting undid an incapacitation or death), the combat runtime reissued a full loadout (refilling scarce P6 ammunition), and the roster owner dropped admission permanently (contradicting the resume policy). Each retained record is consumed on restore so a reconnect cannot be replayed, and all are released when the operation stops. Class selection/resources and the contribution ledger already survived correctly — both keyed by entity ID, neither listening to `PlayerRemoving`. Temporary armor deliberately does not survive. Late joining stays prohibited, and abandonment remains authoritative through the single terminal resolver. Fixture: `OperationSessionRetention.test.luau`.
- [~] **P10-0107 — Validate the full non-persistent match loop.**
  ...
  - **Automated half complete** — `P10MatchLoopValidation.test.luau` proves the composed surface: the complete terminal fact matrix resolves to exactly one cause with the documented precedence, every declared cause is reachable, racing facts after a commit always report `AlreadyResolved`, every cause assembles a valid frozen result that no second commit can replace, a replayed run carries its own identity, every declared cleanup owner maps to a real module with a real teardown and really releases its operation-scoped records, and no P10 owner accepts a client request. **The 1/2/4-operative Studio matrix remains outstanding** and P10 is not signed off until it is recorded; the session script and evidence-capture shape live in [`../production/P10-0107-STUDIO-CHECKLIST.md`](../production/P10-0107-STUDIO-CHECKLIST.md).
...
1. Schedule the **P8-0108** live Studio playtest of the full objective chain (forced relocation and temporary defense). P8 is implemented and fixture-validated through `P8-0107`. **P9 is complete** (`P9-PLAN-001` through `P9-0106`) with the boss-defeat terminal wiring deferred to `P10-0102` and a qualitative Studio gate outstanding. **P10-PLAN-001** through **P10-0102** are complete (see [`../specifications/match-completion-and-result.md`](../specifications/match-completion-and-result.md); the vocabulary and the pure `TerminalResultResolver` are implemented); begin **P10-0103** (integrate the final extraction sequence).
