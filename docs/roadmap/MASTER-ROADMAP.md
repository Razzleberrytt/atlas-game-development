...
| RPG — Run-based roguelite builds | In progress | `RPG-PLAN-001` (`docs/specifications/rpg-integration-plan.md`) defines the RPG-0101→0113 sequence. `RPG-0101` contracts/config and `RPG-0102` operation-scoped run-build centralization are complete. Eight Field Upgrades are implemented under canonical RPG-0101 catalog IDs and ceiling-locked by `RunRpgReconciliation`; expanding that pool to 12–16 remains under `RPG-0103`. Elite affixes, relics, and rewards remain. Some implemented upgrades ship interim mechanics (see plan §7.4). |
