...
| HROI — Horde pressure and run-reward vertical slice | In progress | Bounded slices merged through PR #128 (issue #98): horde pressure and pacing tuning, confirmed hit/kill impact feedback, floating damage text, shooter hit markers, critical-condition urgency, shared run-only Field XP with squad upgrade choices, scarce enemy ammunition/recovery/Field Intel loot, five-weapon loadout roster with cleave/pierce patterns, six readable horde roles, environment mood, and client-local firearm/hostile audio. Later slices grew the run-upgrade pool to a **v4 eight-pick pool** (Trauma Plating, Field Discipline, Combat Loader, Pattern Amplifier) under the canonical RPG-0101 catalog IDs. Representative Studio playtest validation remains the outstanding gate. |
| RPG — Run-based roguelite builds | In progress | `RPG-PLAN-001` (`docs/specifications/rpg-integration-plan.md`) defines the RPG-0101→0113 sequence. `RPG-0101` contracts/config complete (PR #142). Field Upgrade pool expansion (part of `RPG-0103`) is underway ahead of `RPG-0102`: Trauma Plating, Field Discipline, Combat Loader, and Pattern Amplifier are implemented via the HROI run-progression owner under canonical RPG-0101 catalog IDs, ceiling-locked by `RunRpgReconciliation`. `RPG-0102` run-build centralization, elite affixes, relics, and rewards remain. Some implemented upgrades ship interim mechanics (see plan §7.4). |
...
Since PR #128 the run-upgrade pool grew from the four-pick v3 pool to a **v4 eight-pick pool**: added Trauma Plating (bounded squad incoming-damage mitigation consumed at the enemy attack source), Field Discipline (bounded Field XP economy), Combat Loader (bounded reload-duration reduction consumed by `ReloadResolver`), and Pattern Amplifier (bounded shotgun-cleave/sniper-pierce secondary-damage boost consumed in `DamageResolver`'s pattern path). These adopt the canonical RPG-0101 catalog identities rather than off-catalog IDs, and `RunRpgReconciliation` locks every live upgrade to an implemented catalog entry and every live modifier ceiling within the shared RPG-0101 global bounds. Trauma Plating and Field Discipline ship interim mechanics pending the temporary-armor buffer and cooperative-action XP sources their RPG-PLAN-001 forms require; Combat Loader matches its planned mechanic. This work also seeds the RPG track (see the RPG milestone row).
