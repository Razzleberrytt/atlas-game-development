...
| RPG — Run-based roguelite builds | In progress | `RPG-0101` contracts/config, `RPG-0102` operation-scoped run-build centralization, and `RPG-0103` twelve-upgrade pool expansion are complete. Offers are deterministic, weapon-compatible, family-diverse, and ceiling-locked. `RPG-0104` shared modifier centralization is next; elite affixes, relics, and rewards remain. Some implemented upgrades ship interim mechanics (see plan §7.4). |
