...
Snapshot as of 2026-07-22. Task-level detail and acceptance gates for unfinished P6–P12 work live in [`P6-P12-EXECUTION-ROADMAP.md`](P6-P12-EXECUTION-ROADMAP.md), which controls those tasks per the [roadmap index](README.md); the cross-cutting art sequence lives in [`VISUAL-PRODUCTION-TRACK.md`](VISUAL-PRODUCTION-TRACK.md).
...
| P9 — Special enemy and boss | Complete | `P9-PLAN-001` through `P9-0106` complete. The Blight Spitter special enemy and The Progenitor boss are live `EnemyDirectorService` archetypes with readable client telegraphs, server-authoritative and bounded, with automated security/performance/composition validation. Boss-defeat → terminal result landed in `P10-0103`; a qualitative Studio session remains a manual gate. |
| P10 — Match loop and replay | In progress | `P10-PLAN-001` through `P10-0103` complete: the terminal flow is specified, the `MatchResultContracts`/`MatchResultConfig` vocabulary is declared, the pure `TerminalResultResolver` converges every cause on one first-commit-wins boundary with fixed precedence, and the final extraction sequence is live — the holdout is won by defeating The Progenitor rather than by a timer, its authoritative defeat opens the configured arrival window, and one admitted, Alive operative in the clearing at the deadline extracts. Squad wipe and abandonment now reach that same single boundary with their own causes. `P10-0104` is complete: a bounded, inert contribution ledger that the combat, revive, class, and mission owners record already-committed facts into, one validated result snapshot frozen at the terminal boundary, and a debrief screen that explains the outcome, cause, key events, and per-operative contribution while promising no XP, rank, unlock, or replay. `P10-0105` is complete: `OperationLifecycleService` executes the shared documented stop and start orders against each system's existing lifecycle, and a server-driven replay restarts the operation after a configured debrief window with a fresh per-run `operationId`, so a replay is a distinct result rather than a resume. `P10-0106` is complete: session retention now lives in each owning system keyed by operative entity ID, closing three real gaps where a reconnect undid an incapacitation or death, refilled scarce ammunition, or lost admission — each retained record is consumed on restore and released with the operation. `P10-0107`'s automated half is complete — the composed terminal surface is proven across the full cause/race matrix, single-result assembly, cleanup coverage, and a server-only loop. Its 1/2/4-operative Studio matrix remains an outstanding manual gate, so P10 is not signed off. |
...
- **Then:** `RPG-0109` is complete — all six relics landed through their existing owners (Breach Doctrine and Longwatch Doctrine in `DamageResolver`'s pattern pass, Salvager's Mark in `EnemyLootService`'s collection path, Suppression Engine across `AutomaticFireResolver` and `DamageResolver` from a saturating sustained-shot counter, and Guardian Signal and Second Wind inside the P3 life core as expiring temporary armor and a one-charge incapacitation reprieve). All twelve declared relics are now Implemented. `RPG-0110` is complete for every available owner — confirmed elite deaths, a squad-kill milestone, and confirmed special interrupts now open real relic choices through the owners that already observe them, so relics are earnable in a live operation; objective, container, and boss sources stay deferred to P8/P9. `RPG-0111` is complete — an always-visible relic bar with charge and counter state, plus the reward-choice and replacement interfaces, over a bounded `RunBuildNetwork` whose only client requests are the two identity-free relic intents, so relics can finally be earned, chosen, replaced, and read in a live operation; `RPG-0112` is complete — the finished build rides on P10's result snapshot and reads on the debrief screen; `RPG-0113` performs final multiplayer/security/performance validation.
- **Blocked on evidence or dependencies:** no implementable RPG task remains. `RPG-0108`'s "three viable build patterns" gate, `RPG-0113`'s validation matrix, and the HROI representative playtest still need Studio sessions; `RPG-0112` is complete. P7 class-effect runtime is now unblocked, while P8 and later still wait on their prerequisites.
...
- [x] **P7-0105 — Add the engineer vertical slice.** Two finite operation-issued resupply charges channel to the Engineer or a nearby Alive teammate, then commit a compatible capped grant through the existing supply/combat owner and disclose the distinct source to scarcity telemetry; objective repair remains a P8 integration.
- [x] **P7-0106 — Integrate cross-class interactions and squad presentation.** Readable class identity, resources, cooldowns, teammate cues, and concise failure reasons proving the intended cooperation loop without color- or audio-only signals.
- [x] **P7-0107 — Complete class security, scaling, and multiplayer validation.** Adversarial/scaling fixtures, bounded authoritative validation telemetry, and local four-client startup evidence cover the milestone without inventing balance results.

### P7 execution order

`P7-PLAN-001` → `P7-0101` → `P7-0102` → (`P7-0103` + `P7-0104` + `P7-0105`, one PR at a time) → `P7-0106` → `P7-0107`. The full sequence is complete.
...
**P7 status:** Complete. Planning, contracts, selection/assignment, Combat Specialist Brace, Medic treatment/revive assistance, Engineer Field Resupply, cross-class presentation, and final security/scaling validation are complete (`P7-PLAN-001` through `P7-0107`).
...
- [x] **P9-0101 — Define special-enemy contracts, configuration, and pure decisions.** Stable archetype/action/state/rejection vocabulary, tuning values, and a pure resolver for targeting, legal ability use, cooldowns, interruption, death inertness, and tie-breaks. Complete: `SpecialEnemyContracts`, `SpecialEnemyConfig`, and the pure `SpecialEnemyBehaviorResolver` (densest-cluster Corrosive Bloom targeting, the Begin/Continue/Commit charge lifecycle, the in-radius commit burst, and lingering-pool ticks) with fixtures. No runtime yet (`P9-0102`).
- [x] **P9-0102 — Integrate the special enemy into the production director.** Reuse the existing enemy identity/health/spawn/damage/cleanup/stand-down owner and bounded evaluation; no per-enemy scheduler or client authority. Complete: `EnemyDirectorService` owns the Blight Spitter as an archetype with the Corrosive Bloom charge/commit/lingering-pool lifecycle committed through the P3 boundary, authored rarity within the population budget, and stand-down/teardown cleanup — no new connections, timers, raycasts, remotes, or randomness.
- [x] **P9-0103 — Define boss contracts, configuration, and phase resolver.** Stable phase/transition/vulnerability/attack/summon/objective/outcome vocabulary with pure deterministic phase transitions from server-owned facts. Complete: `BossContracts`, `BossConfig`, and the pure `BossPhaseResolver` for The Progenitor — monotonic `Carapace → Brood → Collapse → Defeated` phases, post-Slam vulnerability windows, the Slam AoE, the one-shot Brood summon, and the shroud/Collapse-darkness visibility override (P8 floodlights pay off in Collapse) — with fixtures. No runtime yet (`P9-0104`).
- [x] **P9-0104 — Implement the boss runtime and authored arena integration.** One boss instance owned through production enemy/operation boundaries with revision-safe, bounded, cleaned-up phases, attacks, adds, and terminal state. Complete: `EnemyDirectorService` owns the Progenitor as an archetype (authored `spawnBoss` at the extraction clearing, `applyBossBehavior` driving phases/Slam AoE/Brood adds/shroud through `BossPhaseResolver`), boss fire is vulnerability-gated in `commitHealthState`, the shroud shrinks combat visibility via `readGameplayVisibilityRadiusStuds`, `MissionDirectorService` spawns the boss at holdout with the P8 floodlight fact, and defeat/stand-down/teardown are handled. Boss-defeat → terminal result is deferred to `P10-0102`; telegraphs are `P9-0105`.
- [x] **P9-0105 — Add readable telegraphs and accessible presentation.** Redundant position/timing/shape/text/animation/audio cues for every dangerous action without early disclosure or client-legalized hits. Complete: `SpecialEncounterTelegraphConfig` plus the client `SpecialEncounterTelegraphController` render the Spitter Corrosive Bloom (with lingering footprint), the Progenitor Slam, and the boss phase / armored-vs-exposed status as server-disclosure-driven ground discs with countdowns and text — one frame loop, fixed pool, no remotes/health-reads/hit prediction.
- [x] **P9-0106 — Complete encounter security, performance, and class-composition validation.** Forged-fact/stale/disconnect/wipe/replay/cleanup coverage, representative 1/2/4 horde-plus-boss profiling, and Studio proof that counterplay is attributable. Complete (automated): the per-resolver and director fixtures cover forged/stale facts, vulnerability gating, the Slam AoE/Brood surge, and defeat/stand-down/teardown cleanup, and `P9EncounterSecurityValidation.test` consolidates the milestone guarantees — pure-resolver decisions, P3/revisioned-health damage boundaries, no remote surface, no per-boss scheduler/raycast/randomness, one heartbeat, server-owned shroud, arena spawn, cosmetic telegraph, and 1/2/4 composition bounds. The qualitative Studio profiling/counterplay session remains a manual gate.
...
**P9 status:** Complete. `P9-PLAN-001` specified the special enemy (Blight Spitter) and boss (The Progenitor) encounter in [`../specifications/special-enemy-and-boss-encounter.md`](../specifications/special-enemy-and-boss-encounter.md), reusing the existing `EnemyDirectorService` owner and the single terminal boundary. `P9-0101` then implemented the Blight Spitter's shared contracts, balance configuration, and pure ability resolver (densest-cluster Corrosive Bloom targeting, the charge lifecycle, the in-radius commit burst, and lingering-pool ticks) with fixtures. `P9-0102` integrated the Spitter into `EnemyDirectorService` as a live archetype — archetype-aware fair spawning, the charge/commit/lingering lifecycle committed through the P3 damage boundary, authored rarity bounded by the population budget, and stand-down/teardown cleanup — with no new connections, timers, raycasts, remotes, or randomness. `P9-0103` then defined the boss: `BossContracts`, `BossConfig`, and the pure `BossPhaseResolver` for The Progenitor (monotonic phases, post-Slam vulnerability windows, the Slam AoE, the one-shot Brood summon, and the shroud/Collapse-darkness visibility override that pays off the P8 floodlight repair). `P9-0104` integrated the boss runtime into `EnemyDirectorService` — an authored single instance at the extraction clearing, phase/Slam/Brood/shroud driven by the resolver on the shared evaluation pass, vulnerability-gated fire, the shroud wired into the combat visibility provider, and holdout spawn via `MissionDirectorService` — with defeat/stand-down/teardown handled and the boss-defeat → terminal-result convergence deferred to `P10-0102`. `P9-0105` then added the readable client telegraphs — `SpecialEncounterTelegraphConfig` and the client `SpecialEncounterTelegraphController` render the Spitter Corrosive Bloom (with its lingering footprint), the Progenitor Slam, and the boss phase / armored-vs-exposed status as server-disclosure-driven ground discs with countdowns and text, on one bounded frame loop with no remotes, health reads, or hit prediction. `P9-0106` then closed the milestone with automated encounter validation — `P9EncounterSecurityValidation.test` plus the per-resolver and director fixtures prove the composed surface stays server-authoritative and bounded (pure-resolver decisions, P3/revisioned-health damage boundaries, no remote surface, no per-boss scheduler/raycast/randomness, one heartbeat, server-owned shroud, arena spawn, cosmetic telegraph, 1/2/4 composition bounds). The boss-defeat → terminal-result convergence is deferred to `P10-0102`, and a qualitative Studio session (attributable counterplay, resource-choice respect, 1/2/4 profiling, telegraph mix) remains a manual gate. The next milestone is P10.
...
- [x] **P10-0101 — Define match-result and extraction contracts/configuration.** Stable result/cause/extraction/readiness/cleanup/replay vocabulary and safe result snapshots containing only server-authored operation facts. Complete: `MatchResultContracts` (cause vocabulary + fixed cause→outcome pairing, extraction-readiness/replay/cleanup-owner IDs, result and per-operative contribution snapshot validators, no XP) and `MatchResultConfig` (extraction arrival window, documented cleanup stop order) with fixtures.
- [x] **P10-0102 — Implement one authoritative terminal-result resolver.** Every terminal cause converges on one first-commit-wins boundary; duplicates and races cannot produce multiple results. Complete: the pure `TerminalResultResolver` resolves the server-owned facts to one result with fixed `SquadWipe > Abandoned > Extracted` precedence and an `AlreadyResolved` guard, with fixtures.
...
**P10 status:** In progress. `P10-PLAN-001` is complete — the terminal operation flow and result semantics are specified in [`../specifications/match-completion-and-result.md`](../specifications/match-completion-and-result.md), reusing the single first-commit-wins terminal boundary and existing owners. Boss defeat becomes the authored success path (finally wiring the P9-0104 deferral into `P10-0102`), and replay mints a fresh per-run operation identity. `P10-0101` then declared the terminal vocabulary — `MatchResultContracts` (cause→outcome pairing, extraction-readiness/replay/cleanup-owner IDs, and the result and per-operative contribution snapshot validators, carrying no XP) and `MatchResultConfig` (extraction arrival window, documented cleanup stop order) — with fixtures. `P10-0102` implemented the pure `TerminalResultResolver`: every cause (`SquadWipe`, `Abandoned`, `Extracted`) converges on one first-commit-wins decision with fixed precedence (a dead squad cannot extract) and an `AlreadyResolved` guard, so duplicates and races cannot produce two results. `P10-0103` integrates the boss-defeat extraction sequence next.
