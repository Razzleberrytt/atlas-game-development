...
- **Then:** `RPG-0109` is complete — all six relics landed through their existing owners (Breach Doctrine and Longwatch Doctrine in `DamageResolver`'s pattern pass, Salvager's Mark in `EnemyLootService`'s collection path, Suppression Engine across `AutomaticFireResolver` and `DamageResolver` from a saturating sustained-shot counter, and Guardian Signal and Second Wind inside the P3 life core as expiring temporary armor and a one-charge incapacitation reprieve). All twelve declared relics are now Implemented. `RPG-0110`–`RPG-0111` next add production reward sources and UI as their authoritative gameplay owners become available; `RPG-0112` waits for P10's result owner; `RPG-0113` performs final multiplayer/security/performance validation.
