...
- **Then:** `RPG-0109` is in progress — its weapon-pattern slice (Breach Doctrine, Longwatch Doctrine) is delivered inside `DamageResolver`'s existing pattern pass and Salvager's Mark inside `EnemyLootService`'s collection path, with Suppression Engine, Guardian Signal, and Second Wind following as their owners become usable. `RPG-0110`–`RPG-0111` then add production reward sources and UI as their authoritative gameplay owners become available; `RPG-0112` waits for P10's result owner; `RPG-0113` performs final multiplayer/security/performance validation.
