...
| RPG — Run-based roguelite builds | In progress | `RPG-0101` contracts/config, `RPG-0102` operation-scoped run-build centralization, `RPG-0103` twelve-upgrade pool expansion, and `RPG-0104` shared bounded modifier resolution are complete. Offers are deterministic, weapon-compatible, and family-diverse; all current and planned modifier categories have fixture-locked canonical ceilings. `RPG-0105` elite affixes are next; relics and rewards remain. Some implemented upgrades ship interim mechanics (see plan §7.4). |
