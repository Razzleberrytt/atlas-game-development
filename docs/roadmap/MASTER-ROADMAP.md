...
- **In progress:** `RPG-0108` activates the first six relics (Blood Battery, Grave Momentum, Choir Breaker, Last Light, Emergency Chamber, Execution Protocol). Part 1 landed the pure `RelicModifierResolver` + `RelicRuntimeConfig` foundation; part 2 threads those per-operative modifiers through the authoritative combat/reload/life core (today squad-global via `ProgressionNetwork`) and requires Studio evidence for its "three viable build patterns" gate.
