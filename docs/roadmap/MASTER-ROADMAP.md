...
| RPG — Run-based roguelite builds | In progress | `RPG-0101`–`RPG-0105` are complete: contracts/config, operation-scoped run-build state, the twelve-upgrade pool, shared bounded modifier resolution, and the first Frenzied elite vertical slice. Frenzied assignment is deterministic and server-owned, respects the protected opening and three-elite cap, publishes safe identity, adds a readable telegraph, applies bounded movement/cadence/durability changes, and awards one-shot bonus Field XP. `RPG-0106` completes the elite roster next; relics and later rewards remain. Some implemented upgrades ship interim mechanics (see plan §7.4). |
