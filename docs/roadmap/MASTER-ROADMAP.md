...
| P10 — Match loop and replay | In progress | `P10-PLAN-001` and `P10-0101` complete: the terminal flow is specified and the `MatchResultContracts`/`MatchResultConfig` vocabulary (cause→outcome pairing, extraction-readiness/replay/cleanup IDs, result/contribution snapshots) is declared and fixture-tested. `P10-0102` (the authoritative terminal-result resolver) is next. |
