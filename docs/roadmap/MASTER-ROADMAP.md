...
- **Then:** `RPG-0109` is in progress — all four weapon relics are delivered inside their existing owners (Breach Doctrine and Longwatch Doctrine in `DamageResolver`'s pattern pass, Salvager's Mark in `EnemyLootService`'s collection path, Suppression Engine across `AutomaticFireResolver` and `DamageResolver` from a saturating sustained-shot counter), leaving the cooperative pair Guardian Signal and Second Wind. `RPG-0110`–`RPG-0111` then add production reward sources and UI as their authoritative gameplay owners become available; `RPG-0112` waits for P10's result owner; `RPG-0113` performs final multiplayer/security/performance validation.
