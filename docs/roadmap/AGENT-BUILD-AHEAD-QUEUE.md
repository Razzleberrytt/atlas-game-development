...
| BA-042 | DONE | Loot/build-decision coverage audit. | [`docs/specifications/loot-build-decision-coverage-audit.md`](../specifications/loot-build-decision-coverage-audit.md) — finds the product explicitly rejects rarity/set/stat-roll itemization (`rpg-integration-plan.md` §21); real gaps are 5 stubbed Field Upgrades blocked on missing fact sources, a stale P9 boss-reward dependency note (P9 is done; should read P10-0102), and outstanding RPG-0108/RPG-0113 Studio-evidence gates. No runtime activation. |
| BA-043 | BLOCKED — premise conflict, needs re-scope decision | Deterministic item/affix generation rules. | As originally written this assumes a rarity/affix itemization system BA-042 confirms the product explicitly excludes (`rpg-integration-plan.md` §21). Do not implement as-is. Before starting: either retarget to the real BA-042 follow-ups (flip a `Planned` Field Upgrade once its fact source lands; wire the boss relic-reward source once P10-0102 lands) or have a human confirm itemization is now in scope. |
