...
- The existing death billboard, blood-pool Part, Debris cleanup, and camera-shake channel are reused.
...
- source audit proving server-owned cancellation, preserved kind, `Interrupted` state, unchanged sequence, no reinforcement commit, telegraph deactivation, and zero new authority.
