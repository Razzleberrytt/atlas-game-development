...
The later combat-readability correction raises the pre-dawn ambient floor, reduces atmosphere density and color-grade contrast, and moves fog outside the immediate combat camera. Threat response may still cool and desaturate the scene, but its maximum darkening is deliberately small so high threat cannot crush terrain and enemy silhouettes. The default tactical height is now 68 studs with a `-56` degree pitch instead of the original 92-stud, `-60` degree view, keeping combatants large enough to parse and adding stronger near-to-far parallax without removing mouse-wheel zoom.
...
- A one-client combat review found that the 92-stud default camera, low exposure, dense haze, and threat-grade contrast compounded into a muddy image. The closer 68-stud default, slightly lower viewing angle, and bounded lighting correction preserve the night identity while restoring route, surface-detail, and silhouette readability.
