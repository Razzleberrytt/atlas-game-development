...
LK-0304 adds a separate pure `OperativeReviveResolver`. `beginRevive(reviverSnapshot, targetSnapshot, authorityFacts)` creates an `Active` session only for distinct valid identities, an `Alive` reviver, an `Incapacitated` target with no accepted active owner, finite nonnegative distance at or below the configured `8` studs, server-confirmed line of sight, and a valid authoritative timestamp. The resolver derives the completion deadline as start plus the configured four seconds; clients provide none of these authoritative facts.

`continueRevive(reviverSnapshot, targetSnapshot, sessionState, authorityFacts, combatState)` derives normalized progress solely from authoritative evaluation time and the stored canonical timestamps. It revalidates session ownership and participants, both life states, server-owned connected/participating, movement, accepted-damage, and hold-continuity facts, range, and line of sight. A continuity failure returns a newly copied `Cancelled` session at zero progress; no progress is banked, resumed, or decayed, and a retry needs a fresh start and deadline.
...
## LK-0307 server-owned squad-failure evaluation

`SquadFailureEvaluator.evaluateViability` accepts only copied operative snapshots, explicit operation participation/abandonment facts, and a server timestamp. At least one registered `Alive` operative is viable. With no Alive operative, viability exists only for one Incapacitated solo operative whose pending automatic recovery is eligible, unused, canonically timed, and not invalidated by any multiplayer participation. Incapacitated or Dead teammates never imply a reviver, disconnected snapshots are absent, zero admitted connections after teardown are abandoned, and theoretical future or late joining is not a recovery path.

`SquadFailureEvaluator.resolve` owns the stable `Viable`, `Pending`, and `Failed` transition policy. First non-viability stores the authoritative start and the exact start-plus-`OperativeLifeConfig.SquadFailureGraceSeconds` deadline. Before the stored deadline it remains Pending. Viability before the deadline cancels Pending, clears both timestamps, and banks no partial grace. At or after the stored deadline it commits Failed; Failed is idempotent and ordinary viability cannot cancel it. Invalid facts/state/time and stale evaluation timestamps are deterministic rejections.

`SquadFailureService` owns one same-server failure state/revision and the minimum operation participation facts. Explicit operation start freezes admitted identities, records whether it started solo or with multiple participants, configures the narrow solo eligibility seam in `OperativeLifeService`, prohibits later participation, and ignores later registrations for viability. It reevaluates copied snapshots on life commits, registration, and removal and uses one cancellable server timer only for deadline completion. Final disconnect marks abandonment; duplicate removal is harmless and reconnect restoration is deferred. No RemoteEvent exposes failure mutation or timing.

## LK-0308 runtime integration boundary

`OperativeLifeRuntimeService` is the one owner of stored bleed-out and solo-recovery deadline evaluation. It schedules only the earliest deadline, re-reads the current authoritative snapshot and revision when evaluation runs, invokes `OperativeIncapacitationResolver`, and commits through `OperativeLifeService`. The same owner exposes a server-module-only finishing-damage route that requires an Incapacitated target, uses a server-created damage event and timestamp, reuses the snapshot's processed-event set, and cannot be called by a client remote.

`OperativeReviveSessionService` owns active runtime sessions and permits one owner per target and per reviver. Its only client input is the strict, rate-limited `{ targetOperativeEntityId, phase = "Begin" | "End" }` intent. Sender identity, target admission, current snapshots, distance, line of sight, authoritative time, session identity, movement, accepted damage, character, connection, progress, completion, and restored state are server-derived. Release or any canonical interruption destroys the active session and all progress. Completion at or after four continuous seconds commits the resolver's target snapshot and combat companion together, preserving ammunition, cadence, and processed ShotIds while clearing reload and target selection.

Copied operative and squad attributes plus the plain client debug label are temporary validation disclosure. Clients cannot write them back into the authoritative services. Studio-only bindable controls may advance to a stored deadline or invoke authoritative server damage, but use the production resolver and commit path. P3 is complete. Operation results, restart, rewards, extraction, objectives, polished UI, spectating, matchmaking, persistence, and all P4 work remain deferred.
