...
`SquadFailureService` owns one same-server failure state/revision and the minimum operation participation facts. Explicit operation start freezes admitted identities, records whether it started solo or with multiple participants, configures the narrow solo eligibility seam in `OperativeLifeService`, prohibits later participation, and ignores later registrations for viability. It reevaluates copied snapshots on life commits, registration, and removal and uses one cancellable server timer only for deadline completion. Final disconnect marks abandonment; duplicate removal is harmless and reconnect restoration is deferred. No RemoteEvent exposes failure mutation or timing. Operation results, restart, rewards, extraction, objectives, UI, spectating, matchmaking, persistence, solo-recovery scheduling, and P4 remain deferred. LK-0308 is next and remains unstarted.
