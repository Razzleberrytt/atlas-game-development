...
`continueRevive(reviverSnapshot, targetSnapshot, sessionState, authorityFacts)` derives normalized progress solely from authoritative evaluation time and the stored canonical timestamps. It revalidates session ownership and participants, both life states, continuous hold, range, and line of sight. A continuity failure returns a newly copied `Cancelled` session at zero progress; no progress is banked, resumed, or decayed, and a retry needs a fresh start and deadline. At or after the deadline, a valid continuation completes the session at the evaluation timestamp, transitions the target to `Alive`, restores exactly `OperativeLifeConfig.ReviveHealth` (`30`), and clears incapacitation/recovery timing while preserving maximum health, solo recovery eligibility and usage, processed damage IDs, and unrelated state. It grants no invulnerability and does not mutate the reviver or any input.

The LK-0304 APIs are synchronous and unintegrated. Distance and line of sight are server-owned scalar facts calculated by a future runtime owner; this layer contains no positions, instances, raycasts, runtime session storage, timer, polling, Heartbeat, remote, input, UI, character/Humanoid mutation, squad failure, or operation flow. LK-0305 is next and remains unstarted.
