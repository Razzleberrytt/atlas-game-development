...
**Implementation status:** LK-0401 through LK-0408 complete; P4 is complete.
...
### Bounded squad-navigation ping slice

LK-0407 production-connects one Location-only squad ping. `SquadPingService` owns `start`, `stop`, `registerOperative`, `unregisterOperative`, `setOperativeSquad`, `requestPing`, `removePing`, `removeSquad`, `readRecipientSnapshot`, `subscribeRecipient`, and `evaluateExpiriesAt`. The one intent remote accepts exactly `{ categoryId, candidatePosition }`; identity, squad, recipients, timestamps, IDs, limits, canonical payload, and revisions are server-derived. The server validates an authoritative Alive operative and character-root position, rejects non-finite or non-`Vector3` locations, enforces a 120-stud radius, and quantizes sender-relative displacement to 4 studs without extending it. `Teammate`, `Objective`, and `ObservedHostile` remain reserved contract category IDs but are rejected as `InvalidCategory` in this slice. In particular, there is no hostile fallback, entity reference, discovery read, observation refresh, line-of-sight grant, targeting grant, or combat integration.

Accepted pings live exactly 6 seconds, have a 1.5-second per-sender cooldown, replace the sender's previous ping deterministically, and are capped at one active ping per sender and four per squad. One centralized earliest-expiry scheduler removes due pings in stable sequence order; repeated expiry evaluation is idempotent. Server-owned `ping:<sequence>` identities and monotonic revisions establish deterministic ordering and replay-safe full snapshots. Snapshots contain only ping/category identity, quantized location, already-visible squadmate sender identity, expiry, sequence, and revision. They are sent only to currently registered members of the same squad; a late member receives the currently active snapshot. Incapacitation, death, disconnect/removal, squad change/removal, and teardown clear applicable pings, and revival restores none.

`SquadPingController` binds middle mouse or G, gamepad D-pad Up, and a readable touch action; F and gamepad Y remain exclusive to the flashlight. One action sends one Location intent. Approved pings render as a minimal world marker with a diamond shape plus explicit `LOCATION`/`SQUAD PING` text, so color and audio are unnecessary. There is no flash, strobe, minimap, compass, trail, or tracking behavior. Server snapshots and their expiry timestamps drive deterministic creation and cleanup; malformed or stale snapshots are ignored.

Two-client Studio validation observed clean server/client bootstrap, the same one-ping revisioned snapshot for both current squad recipients, and the diamond plus `LOCATION`/`SQUAD PING` marker on both clients after Player2 pressed G. The marker disappeared after the six-second lifetime. A server-harness incapacitation of the actual sender committed `Incapacitated` and synchronously produced zero-ping snapshots at the same revision for both recipients; the incapacitated sender could not create another ping. The session then stopped with the server and both client windows closing cleanly. Replacement, cross-squad isolation, late-join state, disconnect/squad-change cleanup, and the absence of discovery/targeting effects remain deterministic fixture and source-audit assertions because the production slice has one squad and deliberately imports no perception, discovery, targeting, or combat owner.

The frozen rejection taxonomy is grouped and reused without alternate casing or synonyms: sender/state — `ServiceStopped`, `InvalidSender`, `UnknownOperative`, `InvalidLifeState`, `IncapacitatedSender`, `DeadSender`, `InvalidSquad`; intent/payload — `MalformedIntent`, `StaleIntent`, `InvalidCategory`, `InvalidPayload`; data — `InvalidLocation`, `OutOfRangeLocation`, `InvalidEntityReference`, `UndisclosedReference`, `InvalidObservationState`, `StaleObservation`, `MissingDirectSight`, `StaleDirectSight`; limits — `RateLimited`, `SenderLimitExceeded`, `SquadLimitExceeded`; safety/internal — `InternalError`, `RecipientUnavailable`, `StateMismatch`. Hostile-specific identifiers are reserved for any future strictly authoritative direct-sight implementation; because LK-0407 omits that category, its request path fails earlier with `InvalidCategory` and cannot disclose or refresh anything.

### LK-0408 final validation

LK-0408 completes P4 without changing a production service. The new cross-system fixture composes the production `VisibilityResolver`, `DiscoveryMemoryService`, and P2 `TargetCandidateValidator`, audits the production flashlight and ping remote surfaces, and measures the existing bounded ownership model. All earlier fixtures pass unchanged. No measurable issue justified an optimization or gameplay repair.

Security and adversarial validation failed closed as follows:

| Attempt | Authoritative result |
| --- | --- |
| Create an observation, refresh memory, or force `DirectSight` | No client route exists. Forged visibility results fail correlation as `InvalidVisibilityResolution`; stale timestamps fail as `StaleObservation`; loss-of-sight input preserves the original observation time and bounded expiry. |
| Create discovery or refresh discovery through a flashlight or ping | Flashlight and ping owners import no visibility, discovery, perception, or combat owner. Flashlight coverage with consequential LOS false produces neither direct sight nor targeting visibility; Location ping snapshots contain no observed entity or discovery field. |
| Spoof flashlight owner, operative, light/source/profile/geometry, timestamp, revision, or target | The boolean-only remote rejects extra arguments and all table/oversized payloads as malformed; player-to-operative ownership, life state, server time, deterministic light ID, descriptor, cooldown, and revision remain server-derived. |
| Spoof ping sender, operative, squad, ID, category, entity, recipient, timestamp, or revision | The exact two-field Location intent rejects missing/extra/oversized fields, arbitrary or reserved categories, hostile references, and foreign players. Server state derives sender, squad, `ping:<sequence>`, recipients, time, quantization, expiry, and revision. |
| Submit `NaN`, infinity, malformed values, hostile references, or out-of-range positions | Non-finite/non-`Vector3` positions fail `InvalidLocation`; positions beyond 120 studs fail `OutOfRangeLocation`; reserved hostile categories fail `InvalidCategory` before any entity or discovery lookup. |
| Exceed cooldowns, gameplay-light budgets, sender ping limits, or squad ping limits | Flashlight cooldown and the eight-active-light ceiling, ping 1.5-second cooldown, one-per-sender replacement, and four-per-squad ceiling reject without state/revision mutation. |
| Bypass incapacitated/dead restrictions or replay removed identities | Life commits synchronously force lights off and remove pings; later input fails `NotAlive`, `IncapacitatedSender`, or `DeadSender`. Removal invalidates ownership and stale lifecycle callbacks cannot affect replacement registrations. |
| Create targeting authority or bypass P2 combat gates | Illumination coverage without consequential LOS resolves non-targetable. P2 separately rejects `NotVisible` and `NoLineOfSight`; only an authoritative visible/LOS result reaches the existing combat validator. |
| Mutate copied snapshots, alter revisions, or replay stale callbacks | Mutating retained inputs, safe payloads, subscriber payloads, and copied reads does not change authority. Equivalent replay is silent, stale transitions/revisions/timestamps reject, and scheduler callbacks revalidate generation, identity, revision, state, and deadline. |
| Broadcast to unauthorized recipients | Flashlight state is owner-only. Ping state uses current server-owned squad membership and never `FireAllClients`; cross-squad, removed, and disconnected recipients receive no state. Discovery records are recipient-specific and have no remote. |

The runtime/authority audit found four P4 `RemoteEvent`s and no P4 `RemoteFunction`: flashlight `ToggleIntent`/owner `State`, and ping `Intent`/recipient `State`. Only the two intent events have server listeners. Gameplay lighting and discovery remain server-only owners; visibility remains a side-effect-free shared resolver supplied with server facts; P2 independently owns target validation. The ownership direction is life → flashlight/ping cleanup, flashlight → gameplay-light activation, authoritative visibility → discovery commit, and visibility truth → P2 validation, with no reverse dependency or cycle. Centralized expiry owners cancel and revalidate stale callbacks. Service/controller stop paths disconnect subscriptions, cancel timers, destroy harness/presentation instances, and clear registrations, records, lights, pings, and listeners. Fixtures cover teardown, respawn/replacement, reconnect/disconnect, squad change/removal, operation failure/shutdown, replay, stale callbacks, and copied-read mutation.

Representative normal-operation measurements use one active personal flashlight and one active Location ping per operative in the production-connected owners; discovery remains intentionally unbootstrapped, so production memory tables and visibility update loops are zero:

| Players | Gameplay lights | Active pings | P4 expiry schedulers | P4 connections | Memory tables | Shadows / particles | Visibility rays/checks |
| ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| 1 | 1 | 1 | 1 | 15 | 0 | 0 / 0 | 0 / 0 |
| 2 | 2 | 2 | 1 | 23 | 0 | 0 / 0 | 0 / 0 |
| 4 | 4 | 4 | 1 | 39 | 0 | 0 / 0 | 0 / 0 |

The connection total is seven constant server P4 connections plus eight per client; it does not grow through duplicate starts. Permanent flashlight activations schedule no timer. All pings share one earliest-expiry timer, and discovery would own one additional timer only while position-free stale memory exists. Flashlight and ping state update only on input/lifecycle events; ping presentation performs one cleanup heartbeat per client. There is no production visibility polling, enemy iteration, raycast, discovery replication, shadow-casting P4 light, or P4 particle. The four-player light count is half the gameplay-light ceiling and the four pings exactly meet, but do not exceed, the squad ceiling. Expiry, disconnect, removal, and stop fixtures return counts to zero. No premature optimization was applied.

Direct Studio observation used one server and two clients. Both clients and the server bootstrapped without project runtime errors. Client input showed accessible `Flashlight ON/OFF`, `PING`, and `LOCATION / SQUAD PING` presentation; the ping expired from presentation. Server-driven Player1 incapacitation was observed as `Incapacitated 0/100`, `Flashlight OFF`, and `PING UNAVAILABLE`, while Player2 remained `Alive 100/100` with `Flashlight ON`. Player1 subsequently displayed `Dead 0/100`, with both aids unavailable/off. Live server snapshots measured `2 players / 2 flashlight states / 2 gameplay lights / 1 active light / 2 ping recipient tables`; after Player1 disconnected they measured `1/1/1/1/1`, and after operation client shutdown `0/0/0/0/0`. The server and both client windows closed cleanly with no duplicate instance or leaked-owner evidence. The returned `didCommit` field from the already-incapacitated `Kill` harness call was false even though the finishing transition was visibly committed to `Dead`; that reflects the harness result shape and is not treated as an error.

Four-player Studio load was not run. Valid revival staying off, respawn/re-registration, reconnect, cross-squad removal, malformed-client injection, stale callbacks, and mutation attacks remain fixture-validated rather than directly observed in this LK-0408 Studio session. Earlier LK-0406/LK-0407 Studio runs provide the prior direct two-client malformed-payload and shared-ping observations.

Unrelated tooling notes: the changed fixture passes StyLua and all production source passes Selene, but a repository-wide StyLua check on this Windows checkout proposes LF rewrites for unchanged CRLF Luau files; no line-ending normalization was included. Direct Selene invocation on the repository's `*.test.luau` multi-dot fixture names misclassifies them as plain Lua and reports Luau syntax as parse errors, so fixtures remain Lune-executed and production `src` remains the Selene scope. Rojo sourcemap/build and `git diff --check` pass.

Known prototype limitations remain intentional: visibility and discovery have no production caller or client presentation; the P2 Studio hostiles are development fixtures rather than P5 hidden enemies; flashlight cosmetics are owner-only; the one Location ping uses one production squad; reconnect restoration, production operation flow, network latency/packet-loss profiling, mobile/console device testing, and representative hostile-load ray measurements are absent. P5 may add enemy spawning, pursuit, attacks, authored pressure, and representative hostile-load visibility profiling, but it must preserve these authority, disclosure, budget, and cleanup boundaries. No P5 work begins in P4.
...
LK-0407 selects only the bounded Location ping described above. Other navigation tools and ping categories remain deferred. Every future selected tool still needs an information-disclosure rule, server-owned state if it affects gameplay, rate/placement/lifetime limits, and an equivalent non-audio cue where necessary.
