...
while off+16<=len(b):
    ...
    if clen:
        try:
            ...
            print('decomp fail',sig,clen,ulen,e); raise
    else: data=raw
    if len(data)!=ulen:
        print('len mismatch',sig,len(data),ulen)
    chunks.append((sig,data,off,clen,ulen))
    off=payload_off+(clen if clen else ulen)
    if sig=='END': break
print('chunks',len(chunks), 'types', {s:sum(1 for x in chunks if x[0]==s) for s in sorted(set(x[0] for x in chunks))})

classes={}; ids={}; class_by_inst={}
...
for cid in sorted(classes):
    c=classes[cid]
    print(cid,c['name'],len(c['ids']))

shared=[]
for sig,data,*_ in chunks:
    if sig=='SSTR':
        o=0; ver=i32(data,o);o+=4; n=u32(data,o);o+=4
        for _ in range(n):
            h=data[o:o+16];o+=16; s,o=read_string(data,o); shared.append(s)
        print('shared strings',n,'version',ver)
...
for sig,data,*_ in chunks:
    ...
    if not c: continue
    n=len(c['ids'])
    should = (pn=='Name') or (c['name'] in script_classes and pn in selected_names)
    ...
    for rid,val in zip(c['ids'],vals):
        props.setdefault(rid,{})[pn]=val

parent={}
for sig,data,*_ in chunks:
    if sig=='PRNT':
        o=0; ver=data[o];o+=1; n=u32(data,o);o+=4
        ch,o=refs(data,o,n); pa,o=refs(data,o,n)
        parent.update(zip(ch,pa))
        print('parents',n,'ver',ver)

# names fallback class
...
for rid,cn in class_by_inst.items():
    if cn in script_classes:
        ...
        src=p.get('Source','')
        scripts.append({
            'id':rid,'class':cn,'name':names.get(rid),'path':path_for(rid),
            'disabled':p.get('Disabled'), 'enabled':p.get('Enabled'), 'run_context':p.get('RunContext'),
            'source_len':len(src) if isinstance(src,str) else None,
            'source_sha256':hashlib.sha256(src.encode()).hexdigest() if isinstance(src,str) else None,
            'source':src,
        })
print('\nSCRIPTS',len(scripts))
for s in sorted(scripts,key=lambda x:x['path']):
    print(s['class'],s['path'],'len',s['source_len'],'disabled',s['disabled'],'ctx',s['run_context'])

out={'metadata':{'file':PATH.name,'size':len(b),'sha256':hashlib.sha256(b).hexdigest(),'version':version,'class_count':class_count,'instance_count':inst_count},
     'classes':[{**{'class_id':cid},**c} for cid,c in sorted(classes.items())],
     'scripts':scripts,
     'roots':[{'id':rid,'class':class_by_inst[rid],'name':names[rid],'path':path_for(rid)} for rid in class_by_inst if parent.get(rid,-1)==-1],
     'instances':[{'id':rid,'class':cn,'name':names[rid],'parent_id':parent.get(rid,-1),'path':path_for(rid)} for rid,cn in class_by_inst.items()]}
